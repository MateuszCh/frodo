(function() {
    ...
    function PostTypeController(
        ...
        var vm = this;
        vm.$mdMedia = $mdMedia;
        var fieldsElement = angular.element(
            document.querySelector("#postFields")
        );
        vm.$onInit = onInit;
        vm.save = save;
        ...
        function onInit() {
            if (
                $state.current.name === "postTypesEdit" ||
                $state.current.name === "componentsEdit"
            )
                vm.edit = true;
        ...
        function addField() {
            var html =
                '<add-field model="vm.model.fields" form="vm.form"></add-field>';
                ...
                promise
                    .then(function(response) {
                        ...
                        if (vm.edit) {
                            ...
                            vm.currentType = vm.model.type;
                            tools.infoDialog(
                                vm.model.type + " updated successfully",
                                ev
                            );
                        } else {
        ...
        function removeDialog(ev) {
            tools.removeDialog(
                ev,
                remove,
                "Are you sure you want to delete " + vm.model.type + "?"
            );
        }
