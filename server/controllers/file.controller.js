...
function checkFile(file, cb) {
    if (filenames.indexOf(file.originalname) > -1) {
        ...
        cb(
            `You are trying to upload two files with the same filename: ${
                file.originalname
            }`
        );
    }
...
module.exports = {
    ...
    setFilenames(req, res, next) {
        ...
        File.find({})
            ...
            .catch(e =>
                res
                    .status(422)
                    .send({ error: "There was error uploading files" })
            );
    },
    create(req, res, next) {
                            ...
                            if (fileProps[prop]) {
                                if (prop === "catalogues") {
                                    if (Array.isArray(fileProps[prop])) {
                                        model[prop] = fileProps[prop].map(
                                            catalogue => catalogue.toLowerCase()
                                        );
                                    } else {
                                        model[prop] = [
                                            fileProps[prop].toLowerCase()
                                        ];
                                    }
                                } else {
        ...
        File.findOne({ _id: fileProps._id })
            .then(existingFile => {
                if (existingFile) {
                    ...
                    existingFile.position = fileProps.position;
    ...
    importFiles(req, res, next) {
        const correctFiles = req.body.files.filter(file => {
            const filenameConflict =
                filenames.indexOf(file.filename) > -1 ||
                currentlyUploading.indexOf(file.filename) > -1;
            ...
            Counter.findOne({})
                .then(counter => {
                    ...
                    correctFiles.forEach((file, i) => {
                        ...
                        if (!file.created) model.created = Date.now();
                    ...
                    File.create(filesModels)
                        .then(files => {
                            Counter.update(counter, {
                                $inc: { counter: filesModels.length }
                            })
                                .then(() => {
    ...
    exportFiles(req, res, next) {
        File.find({})
            .then(files => {
                const formattedFiles = JSON.stringify(
                    files.map(file => {
                        const properties = [
                            "title",
                            "description",
